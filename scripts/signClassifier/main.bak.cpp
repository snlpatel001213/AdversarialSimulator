...
public:
    ...
    bool onInitialize() override
    {
        ...
        //------------------------------------------------------------------------------
        {
...
#endif
                {
                    if (getArgument("input-type").compare("image") == 0)
                    {
            ...
            if (getArgument("input-type").compare("image") == 0)
            {
                read_directory(getArgument("imagedir"), imageList);
            }
            else
            {
                if (m_camera == nullptr)
            ...
            if (getArgument("input-type").compare("image") == 0)
            {
                std::cout << "Image mode: " << imageList.size() << " Image loaded from " << getArgument("imagedir") << "" << std::endl;
            }
            else
            {
...
private:
    ...
    void getNextFrameImage(dwImageCUDA **nextFrameCUDA, dwImageGL **nextFrameGL)
    {
        ...
        if (imageList.size() > 0)
        {
            std::cout << "reading file " << getArgument("imagedir") + "/" + imageList[count] << std::endl;
            dwRenderEngine_createImageGLFromFile(&m_sampleImage, (getArgument("imagedir") + "/" + imageList[count]).c_str(), m_renderEngine);
...
#endif
                              ProgramArguments::Option_t("imagedir", (DataPath::get() + "/image").c_str(), "path to image"),
                              ProgramArguments::Option_t("input-type", "video", "input type either video or camera or image"),
